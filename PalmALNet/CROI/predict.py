...
def rotateImage(img, degree, x, y, pt1, pt2, pt3, pt4):
    ...
    heightNew = int(
        width * math.fabs(math.sin(math.radians(degree)))
        + height * math.fabs(math.cos(math.radians(degree)))
    )
    widthNew = int(
        height * math.fabs(math.sin(math.radians(degree)))
    ...
    imgRotation = cv2.warpAffine(
        img, matRotation, (2000, 2000), borderValue=(255, 255, 255)
    )
    pt1 = list(pt1)
    pt3 = list(pt3)
...
def draw2(filename, result):
    img = Image.open(filename)
    w, h = img.size
    draw = ImageDraw.Draw(img)
    #for class_name, lx, y, w, h, ang, prob in result:
    for class_name,lx,ly,rx,ry,ang, prob in result:
        result = [
            int((rx + lx) / 2),
            int((ry + ly) / 2),
            int(rx - lx),
...
def draw(path, result):
    # 读取图像
    image = Image.open(path)
    if not result:
        print(
            "cropping and saving fail!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!: "
            + path.split(".")[0]
            + ".png"
        )
        image.save(
            os.path.join(
                path_name_all + "/fail_images", path.split(".")[0] + ".png"
            )
        )
    for class_name, lx, ly, rx, ry, ang, prob in result:
        result = [
        ...
        # 保存结果
        images_save_path = os.path.join("result.png")
        cropped_image.save(
            "result.png"
        )
        print("cropping and saving successfully: " + "result" + ".png")
        break
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("./all_res34_best.pth", map_location=device))
    #model.eval()
    # model.cuda()
